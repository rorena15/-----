// ── STATE ──
let capturedPhotos = []; // up to 6 Image objects
let selectedOrder = []; // indices into capturedPhotos, length 4
let stream = null;
...
function goToStep(n) {
  document
    ...
    .forEach((p, i) => p.classList.toggle("active", i === n - 1));
  [1, 2, 3, 4].forEach((i) => {
...
function renderDots() {
  ...
  if (done >= 6) shootHint.textContent = "6장 완성! 다음으로 넘어가세요 ✅";
    ...
    shootHint.textContent = `${done}/6장 완료 — 카메라를 켜거나 파일을 업로드하세요`;
}
...
const FRAMES = {
  ...
  scrap: {
    ...
    gap: 12,
    borderRadius: 8,
    border: { color: "#6366f1", width: 2, dashed: true },
    label: "✂ SCRAP",
  },
};
...
function drawFrame() {
  ...
  if (bgImage) {
    ...
    roundRect(
      ...
      W,
      H,
      currentFrame === "scrap" ? 16 : currentFrame === "classic" ? 12 : 4,
    );
    ctx.clip();
  ...
  if (f.label) {
    ...
    ctx.fillStyle =
      bgColor === "#1e293b" ? "rgba(255,255,255,0.3)" : "rgba(0,0,0,0.22)";
    ctx.textAlign = "center";
...
canvas.addEventListener("mousedown", function (e) {
  ...
  const idx = stickers.findIndex(
    (s) => Math.abs(px - s.x) < 28 && Math.abs(py - s.y) < 28,
  );
  if (idx >= 0) {
    dragging = idx;
...
canvas.addEventListener(
  ...
  function (e) {
    ...
    const sx = canvas.width / r.width,
      sy = canvas.height / r.height;
    const px = (e.touches[0].clientX - r.left) * sx,
...
function makeQRCanvas(url) {
  ...
  // Use QRCode.js to draw onto a temp div then copy to our canvas
...
function fallbackQR() {
  // Encode a message to scan in browser
  const qrLoading = document.getElementById("qrLoading");
  const qrCanvas = document.getElementById("qrCanvas");
  const tmpDiv = document.createElement("div");
  tmpDiv.style.display = "none";
  ...
  // Use a data URL shortening trick: encode a small "save page" URL
  // Since we can't host the image, QR will link to a web-based QR image decoder
